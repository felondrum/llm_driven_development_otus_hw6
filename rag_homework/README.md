...
# Модель для генерации (легкая, для Mac M1)
docker exec ollama_rag ollama pull llama3.2
```
...
### Модели (оптимизированы для Mac M1)

| Модель | Назначение | Размер | Примечание |
|--------|-----------|--------|------------|
| `all-minilm` | Эмбеддинги | ~80MB | 384 dimensions, мультиязычная |
| `llama3.2` | Генерация | ~2GB | 3B параметров, быстрая на M1 |

### Альтернативные модели

```python
# В src/config.py можно заменить:
GENERATION_MODEL = "mistral"    # 7B, более точная
GENERATION_MODEL = "qwen2.5"    # 7B, хорошая поддержка русского
```
...
**Альтернативы:**
- `t-tech/tnews` - новости на русском
- `bigscience/xlsum` - многоязычный датасет
...
- [x] Similarity metrics: Cosine similarity
...
- [x] Сравнение параметров HNSW (ef = 32, 64, 128, 256)
- [x] Hybrid search (векторный + фильтрация)
